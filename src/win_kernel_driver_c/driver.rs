...
impl WinKernelDriver {
    ...
    pub fn open(&mut self) -> Result<(), String> {
        ...
        let driver_path_c = CString::new(driver_path).unwrap();

        unsafe {
            let device: winnt::HANDLE = fileapi::CreateFileA(
                driver_path_c.as_ptr(),
    ...
    pub fn io(&self, ioctl_code: u32, mut in_buffer: [u32;3], size: u32) -> Result<u64, String> {
        if !self.opened() {
            return Err("Driver not opened!".to_string());
        }
        ...
        let in_buffer_bytes = size * 4;
        let slice_in_buffer = &mut in_buffer[3-size as usize];

        let in_buffer_size = u32::try_from(in_buffer_bytes).unwrap() as DWORD;
        ...
        unsafe {
            let res = ioapiset::DeviceIoControl(
                device,
                ioctl_code,
                slice_in_buffer as *mut _ as *mut c_void,
                in_buffer_size,
